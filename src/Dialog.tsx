...
export function Dialog(props: {
  ...
  return () => (
    <sl-dialog
      use:ref={ref(props.ref)}
      bool:open={props.open}
      prop:noHeader={props.preventClosing ? true : false}
      label={props.label}
      $attr:style={styleMap({
        fontSize: "1rem",
      ...
      on:sl-after-show={props.onAfterShow}
      on:sl-after-hide={() => {
        props.onAfterHide?.();
      ...
      on:sl-request-close={(e: SlRequestCloseEvent) => {
        if (props.preventClosing && e.detail.source === "overlay") {
          e.preventDefault();
        }
        // TODO: add source detection for escape key
